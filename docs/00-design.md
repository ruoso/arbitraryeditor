...
> `libarbc` (the resolution-independent 2D composition library, v0.4.0) as an
...
| D22 | In-app New / Open / Recent | The tool rail (D18 home base) carries the project-entry affordances — **New Project…**, **Open Project…**, and **Open Recent ▸** — realizing §9's "New / Open (a project folder) / … recent projects" verbs. Because a process is single-project for its lifetime (**D19 / A7**), **every** entry point **spawns a new sibling `exec`** (through `editor.project.exec_new`) and **never swaps the in-process session** — the current window stays up (D19's tab analog), even when it holds an untouched scratch project. **Open** picks an *existing* project directory through the OS native folder dialog and hands it to the sibling; **New** composes a *not-yet-existing* target directory (a chosen parent location + a project name) so the sibling's bootstrap create-vs-open branch (D-app_state-6) scaffolds it — no second `Document` is ever minted in the current process. The **recent-projects** list is an MRU of project directories persisted to the **per-user prefs store**, mirroring D21's single **versioned line-oriented text file** published via `platform::FileSystem::atomic_replace`, capped (most-recent-first) and **pruned of vanished / non-project directories on load**; it is cross-project local scratch, never in `project.arbc`. Native folder picking sits behind an app-level seam (A12); the WASM analog opens the File System Access API at that same seam. The dirty indicator and Save / Save As stay with `editor.project.save`. |
| D23 | Minting a camera | Creating a camera is a rail **action**, never a mode (D20), and a mint always derives **both** knobs at once — a frame **and** a resolution consistent with it. D9's independence governs *editing* the two, not leaving one unset, so a mint may never ask the user to type W×H before the camera exists. Two mint verbs share **one derivation rule**: **"frame selection"** (§6) fits the frame to the **axis-aligned union of the selected objects' placed extents**; **"new shot from view"** (§3) fits it to the viewport's current framing. The derived resolution is the framed region at **1 composition unit = 1 pixel**, rounded to whole pixels and clamped to a maximum longer side; the frame is then **expanded — never cropped — about its center** to the rounded resolution's aspect, so pixels stay square (D9's aspect-lock) and nothing that was framed falls outside. Framing an *unscaled* cell exactly therefore reproduces that cell's native pixel count, which is D8's non-destructive promise read back through a camera. **For "new shot from view" that rule collapses to the pane itself:** the framed region *is* what the viewport shows, so the resolution is the pane's **device-pixel size** and the frame is the viewport camera inverted — the rounding and the aspect-expansion are exact no-ops (a pane is already a whole number of square pixels), and the clamp is **not** applied, because a pane is bounded by the display rather than by the composition, so the terapixel mint the clamp guards against cannot arise. Promoting the view therefore keeps the view's **pixel count**, which is what makes the shot, rendered at its own resolution, reproduce what was on screen; the inspector's W×H is the escape hatch for any other size (D9). **Which viewport, when more than one canvas is open (D18):** the **focused** one — the canvas pane the user most recently worked in, remembered across the click that lands on the rail itself, falling back to the lowest-id live pane when no canvas has yet held focus. **"Lowest id" means numerically lowest instance index, not byte-lexicographically lowest:** a view id is `slug#N` with an unpadded decimal `N` (D18's multi-instance Canvas), so `canvas#2` precedes `canvas#10` even though the bytes sort the other way. The canonical order over view ids is **(view type in catalog order, then instance index ascending)**, with any string that is not a well-formed view id sorting after every one that is, by bytes — a total order, so the fallback is deterministic for any set of open panes. Every "lowest-id" / "first live pane" rule in the shell reads *that* order, so the pane a framing-derived verb lands on — and the marker that names it — stays the earliest-minted live canvas however many canvases a session has opened and closed. A pane offers the framing it is **actually showing**: a free-navigation pane offers its own transient camera at its device size, while a pane **looking through** a saved shot offers that shot's framing at the size it occupies on screen — so promoting a look-through pane duplicates the shot at screen resolution rather than minting an incoherent framing. The same focused pane sources a cell insert's provisional placement (A16), so both framing-derived verbs act on the canvas the user is looking at rather than on an arbitrary one. **That pane is shown, not inferred:** the resolved target pane carries a hairline accent border drawn just inside its rect, derived from the **same** resolution rule the verbs consume — so the marker names the *fallback* pane too, and can never disagree with where the verb will actually land. It deliberately does not lean on ImGui's own focus chrome, a title-bar tint that is easy to miss in a two-pane dock and is absent while the rail holds focus; the marker is driven from the sticky hint and therefore persists across the click that lands on the rail. It is passive chrome — no hit-testing, no layout, no effect on the composition or on any export — and it is drawn on a single-canvas dock too, so "bordered pane = where the verb lands" never varies with how many canvases are open. A minted camera is **auto-named** `Camera <n>` (first free `n`, renamed in the inspector), is **not** auto-selected, and does **not** become any canvas's look-through camera — minting is a scene transaction (undoable, D15) while activating a shot is transient session state (D15/D18). The mint is **refused rather than guessed at** when there is nothing to frame: for "frame selection", a selection with no bounded extent (unbounded fill only); for "new shot from view", **no live, sized canvas pane** — D18 lets every canvas be closed, and substituting the composition's own bounds would mint a framing the user never made. |
| D24 | Deep-zoom navigation aids | Beyond reset-to-fit (D2 §3: `F` frames the whole document), the canvas offers **transient-viewport** framing aids — **zoom to selection**, **fit to cell**, **fit to frame** — that recenter the **active viewport camera** on a region so the user does not get lost in unbounded space. Unlike D23's like-named **"frame selection"** *mint* (which adds a **saved camera** — a scene transaction), these write **only the viewport camera's transient framing** (D15): no journal entry, no dirty, not undoable, like pan/zoom. The two verbs share **one geometry** — the axis-aligned union of the selected objects' placed extents (the same `selected_extent` D23's mint consumes) — but differ in effect (view vs. scene). They also share **one gesture**, "frame the current selection in the view": because selection is **kind-agnostic** (D7), that single gesture *is* **fit-to-cell** for a single selected cell, **fit-to-frame** for a single selected camera (**"cameras only"** emerges because only a camera contributes a camera-frame rect — its output rectangle), and **zoom-to-selection** for a multi-selection — so no separate per-aid mode or key is minted. The region is centered and uniformly scaled to fit the pane **at the pane's own aspect** (the positioned generalization of the reset-to-fit camera), **not** re-aspected/rounded to a resolution the way a mint is — it is a view change, so no resolution is derived. With **nothing bounded to frame** (an empty selection, or a selection of only unbounded fills) the aid is **refused — the camera is left unchanged**, never snapped to the document or to identity (the same refuse-rather-than-guess discipline as D23). A list/overview-driven "fit to *this* camera" (framing a camera you point at without selecting it) and the overview's in-panel zoom control are the **overview navigator's** (D6/§5), reusing the same fit primitive; the aid here frames the **selected** camera. The keyboard binding is **provisional** pending the full input map (§11). |
| D25 | Reopen degradation notice | When a project reopens through the workspace-map fast path with content the map could not bind (**A19**) — the one lossy reopen the editor can produce, a **never-saved** project whose `workspace/` held cells or cameras but which has no canonical `project.arbc` to rebuild from — the editor **announces the loss** instead of presenting a silently-empty project. It surfaces as a **one-shot, dismissible notice on open** naming how many objects could not be recovered ("This project's unsaved workspace held **N** object(s) that could not be recovered."). It is **informational, not an error** (A19: the loss is a *value*; the document does open); the notice is a centered modal drawn like the Clean up confirm (D-gc-3) but with a single **Dismiss**, shown **at most once per session** (the count is a stable bootstrap fact, so a per-session latch makes it one-shot) and **never** on a clean map reopen, a rebuild-from-canonical, or a fresh `create_project` (all report a zero count). The count is read from the in-process session's `commands::AppState` through the `dock::ProjectGateway` session-query seam — the same L3→L4 inversion `is_dirty()`/`can_undo()` use (A12/A13), never a `dock`→`commands` edge. This records only what the editor does with an *already-incurred* loss; **preventing** the loss (making the map path bind content) is a cross-repo library seam (A19's future-fix, parking lot), outside the editor's reach. |
| D26 | Welcome on a project-less launch | Launching the editor interactively with **no project** shows a **welcome**, not a throwaway project. D22's entry affordances live *inside* a project window (the rail is home base, D18), so a process with no project has no home base at all — and minting a scratch project just to have something to show (`D-app_state-6`'s bootstrap default, decided before any picker existed) leaves a stray project directory and a live `workspace/` mmap arena behind on every curiosity launch, because the user's first act is New or Open, which **spawns a sibling** (D19/A7) and abandons the scratch process. So the project-less process opens as a **pre-project launcher** (**A22**): one window whose entire content is the welcome, offering **exactly D22's three verbs** — **New Project…**, **Open Project…**, **Open Recent** — over the *same* `dock::ProjectGateway` (A12), the same native folder pick, the same parent-location + typed-name compose for New, and the same pruned MRU prefs store, so a launcher and a project window list the same recents and refuse the same targets. It is **modal in the UX sense** — the launcher window holds no dockspace, no rail, no canvas, so there is nothing behind it to reach — realized as a full-viewport window rather than a `BeginPopupModal`, because New's compose popup must open *above* it and stacking a modal on a modal is the fiddly path to the same picture. Choosing a verb **spawns the sibling and the launcher exits**; it never becomes the project window (D19: one process, one project, for its whole lifetime). **Dismissing without choosing — closing the window, or `Esc` — exits cleanly too:** there is nothing to fall back to, and inventing a fallback is exactly the throwaway project this row removes; a user who wants a scratch project chooses **New**. **Three verbs only, in v1:** no "start with a scratch project" verb (it re-mints what the launcher exists to stop minting), no "skip", and no remembered default / auto-open-last-project (it takes the choice away at the one moment the user is being asked to make it, and D22's MRU already makes the last project one click away). A **headless** no-arg launch is deliberately unchanged — it still auto-creates the scratch project, because a driven run has no user to answer a welcome and the driven path must always hold a `Document`. |
| D27 | Creation targets a path that does not exist | **A project directory is created, never adopted.** D16 makes the directory *be* the project, so the two verbs that bring one into being — **New Project…** and **Save As…** — take a **not-yet-existing** target and the tool makes the directory. A target that **exists at all** is refused: an empty directory, a directory of unrelated files, another project, or a regular file. "Empty is fine" is deliberately *not* an exception — it would leave New and Save As accepting different targets and put the tool in the business of deciding what "empty" means. **Both verbs therefore share one gesture**: pick a **parent location** through the native folder dialog (A12's seam, the same one Open uses) and type a **project name**, composed into the target; there is no picker that can return an existing directory to either verb. This **extends D22's rule from New to Save As** — D16 §9 gave Save As only "copy the directory" with no destination rule, and D22 stated not-yet-existing + parent-plus-name for New alone — and **replaces Save As's earlier native-folder-pick UX**, which under this row could only ever fail. It is one compose modal with two submit verbs (A22's "one implementation of parent + typed name"), so the rail, the welcome launcher (D26) and any future host refuse identically and render the **same single refusal message** — an invalid name and a taken name both mean "type a different name here". **Open is untouched** (it *adopts* an existing project directory — a different verb with a different validation), and so is plain **Save**, whose whole job is to overwrite its own canonical. Because New spawns a detached sibling rather than creating in-process (D19/A7), the refusal is raised **before** the spawn, so the user sees a message instead of a window that never appears. Realized by `editor.project.dir_is_project`. |
| D28 | Cross-session dirty precision on the durable-by-default map reopen | Restoring the durable-by-default fast path (A19 amendment: the reconstructing reopen through `arbc::open_document`) makes the **workspace-mapped open the common reopen**, and the session-scoped dirty baseline (**D-save-4**) starts a mapped open **dirty** — so a project saved cleanly yesterday would reopen showing *modified*, with Save armed and a pointless re-dump on first press. This row extends the session baseline with a **cross-session published-revision sidecar**: `project::save_project` writes `workspace/published.rev` (a decimal `std::uint64_t`) **after** the canonical `atomic_replace` of `project.arbc` durably succeeds, and the mapped reopen reads it and seeds the session **clean** iff the sidecar is present **and** equals the reconstructed document's revision (`OpenedProject::mapped_in_sync`, ferried into `AppState`'s `saved_revision_` at construction beside the `rebuilt_from_canonical` seed). An **out-of-sync / sidecar-absent / pre-v0.4.0** reopen reads **dirty**. The **never-a-false-clean** asymmetry (D-save-4) is preserved by the **write-ordering**: a crash between the two writes leaves the sidecar stale or absent → the reopen mismatches → **dirty** (a safe false-dirty); a false-clean would require the sidecar to name a revision the canonical does not hold, which the ordering forbids. The revision comparison is well-defined because reconstruction is **revision-invariant** — `arbc::open_document` rebinds objects onto existing records and advances no version (pinned in `tests/arbc_pin_test.cpp`). The sidecar lives in `workspace/` because it is **machine-local sync state** (D16, gitignored scratch), read/written through `platform::FileSystem` (A3, WASM-swappable), never the portable core; a Save As publishes into a fresh root with no workspace, so no sidecar is written into the copy. This **supersedes** D-save-4's *"persisting a cross-session baseline is a deliberate non-goal"* clause (and the `app_state.hpp` header comment that stated it): that non-goal's reason for deferring — *"would edit the shipped `open`/`create` path"* — is void, because this leaf edits exactly that path. **No new component, no new DAG edge, no new external dependency** — one new field on `OpenedProject`, one persisted `workspace/` artifact through the existing `project → platform` edge, and one extended seed condition in L1 `commands`. Realized by `editor.project.reconstructing_reopen`. |
| D29 | Scoped editing | Entering a composition (D17) confines **editing**, not just the view: while a composition is entered, an **insert lands in it**, and **delete, selection, and all pointer/keyboard/drag interaction act only on cells within it** — out-of-scope objects are **dimmed** (D17's view half, `editor.canvas.isolation_scope`) **and interactively locked** (`editor.cells.scoped_edit`). This is enforced at two shipped chokepoints: the edit verbs (`scene::add_cell` / `scene::remove_cells`) resolve the **active** composition through `scene::active_composition` instead of hard-coding the root, and the **single** pick assembly adapter (`interact::pick_targets`, A17) drops out-of-scope targets — so click-select, marquee, select-all, gizmo-drag, and frame-selection are confined at once, without per-gesture logic. **Cameras** are project-level viewport tools (D19/A14), not members of a nested composition, so they are **not pickable while entered** — climb the breadcrumb to Root to manipulate them. A scope that names a **vanished** composition (GC'd / undone-away / foreign) **degrades to Root** editing — the same fail-safe the dim (`composition_focus_quad`) and the list (`active_composition`) already use, so canvas, list, and edit targeting can never disagree about whether the scope is live. The edits stay ordinary transactions (D15); entering, climbing, and the fail-safe add no journal entry and never dirty the document. No new component, no new DAG edge, no new external dependency. Realized by `editor.cells.scoped_edit`. |
